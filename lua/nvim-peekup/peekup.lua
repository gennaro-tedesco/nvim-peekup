...
local function get_reg(char) return vim.fn.getreg(char):gsub("[\n\r]", "⏎") end
    ...
    for _, v in pairs(config.reg_chars) do
        if string.match(v, "%d") and get_reg(v) ~= '' then
            table.insert(numerical_reg, v .. ':' ..
                             string.rep(' ', config.on_keystroke.padding) ..
                             get_reg(v))
        end
    end
...
local function floating_window(geometry)
    ...
    local win_height = geometry.height <= 1 and
                           math.ceil(total_height * geometry.height) or
                           total_height
    local win_opts = {
        relative = 'win',
        width = win_width,
...
local function on_keystroke(key, paste_where)
    ...
    if vim.api.nvim_exec('echo search("^' .. search_key .. ':") > 0', true) ~=
        '0' then
        vim.cmd(':silent! /^' .. search_key .. ':')
        vim.cmd(':noh')
        vim.cmd('execute "normal! ^f:' .. config.on_keystroke.padding + 1 ..
                    'lvg_"')
        vim.cmd('redraw')
